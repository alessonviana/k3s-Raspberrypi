...
├── inventory/
│   ├── hosts.yml           # Machine inventory (generated by script - NOT committed)
│   └── hosts.yml.example   # Inventory example (can be committed)
├── group_vars/
│   ├── all.yml             # Global variables (generated by script)
│   └── k3s_cluster.yml     # k3s cluster variables (generated by script)
├── playbooks/
│   ├── site.yml            # Main playbook
│   ├── k3s-master.yml      # Master installation
│   └── k3s-workers.yml    # Workers installation
├── scripts/
│   ├── setup-inventory.sh  # Interactive configuration script
│   └── setup-ssh-keys.sh   # Script to configure SSH keys
└── README.md               # This documentation
```

⚠️ **Important**: The `inventory/hosts.yml` file contains sensitive information (IPs, users) and is in `.gitignore` to prevent it from being committed to GitHub.
...
- `inventory/hosts.yml` - Ansible inventory (⚠️ **WILL NOT be committed** - contains IPs and users)
- `inventory/.passwords.yml` - SSH passwords file (⚠️ **WILL NOT be committed** - contains passwords)
- `group_vars/all.yml` - Global variables
- `group_vars/k3s_cluster.yml` - k3s cluster variables

🔒 **Security**: The files `inventory/hosts.yml` and `inventory/.passwords.yml` are in `.gitignore` and will not be committed to GitHub, as they contain sensitive information (IPs, users, SSH ports and passwords).
...
📋 Masters Configuration (main nodes)
---------------------------------------------
...
📋 Workers Configuration (worker nodes)
-----------------------------------------------
...
✨ **The kubeconfig has already been automatically copied to `~/.kube/k3s-config`!**
...
- ✅ Copies the kubeconfig from the master to your local machine at `~/.kube/k3s-config`
- ✅ Configures the correct master IP in the kubeconfig
- ✅ Sets the correct permissions (0600)
- ✅ Displays instructions on how to use the kubeconfig
...
1. ✅ Restricted permissions on kubeconfig (0600)
2. ✅ Use of variables for configurations
3. ✅ Interactive script for secure configuration
4. ✅ Inventory and passwords protected in `.gitignore` (not committed)
5. ✅ Example file available (`hosts.yml.example`)
6. ✅ Passwords requested securely (not displayed on screen)
...
**💡 Tip**: Run the `setup-inventory.sh` script whenever you need to reconfigure the cluster or add new nodes!
